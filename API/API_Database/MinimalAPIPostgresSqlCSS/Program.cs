...
{
    //Find admins and returns if is found.
...
app.MapPost("/Admins/Auth",  (Admins a, CSSDb db) =>
{
    ...
    //We are using Admin_Email as the key sending it from the frontend & the password cant be null. 
    if(privatekey != a.Admin_Email || a.Admin_Password == null) return Results.Unauthorized();

    //Search the unique admin_password.
    var pas = db.Admins.OrderBy(x => x.User_Id).Last().Admin_Password;

    if (pas != null)
    {
        //Decrypt the pas admin password.
        if (Crypt.Decrypt(pas) == a.Admin_Password)
        {
            //If is the same return OK response.
...
{
    //Verify that the id exist on the database.
...
{
    //Query to search commands executed by an specific admin id.
...
{
    //Search where the string is the same as the commands post and if performed is null also have same value.
    var properties = (
    ...
    //If is nothing is found the post will be posted.
    if (properties == null)
    {
        db.Commands.Add(a);
        db.SaveChanges();
    ...
    // If something is found the timestamp will be refresh. 
    else
    {
...
{
    //Return the command if found if not the response will be NotFound. 
...
{
    //Returns all the commands that are received.
    var properties = (
...
{
    //Verify that the id selected ar the same as json. 
    if (a.Command_Id != id) return Results.BadRequest();

    //Search for it.
    var Commands = await db.Commands.FindAsync(id);

    //If not found the response will be NotFound. 
    if (Commands is null) return Results.NotFound();
...
{
    //Search for all the commands that are not Retrainmodel also have performed empty.
    var properties = (
                        from Commands1 in db.Commands.DefaultIfEmpty()
                            select new
                            {
    ...
    //if nothing was found it will return nocontent
    if (properties == null) return Results.NoContent();
    ...
    //Update the timestamp of Command_Read
    Commands.Command_Read = a.Command_Read;

    //Save db data.
    await db.SaveChangesAsync();

    //Search the last update that acomplish all the process above.
    properties = (
                            from Commands1 in db.Commands.DefaultIfEmpty()
                            select new
                            {
...
{
    //Search for all the commands that are retrainmodel also have performed null.
    var properties = (
                    from Commands1 in db.Commands.DefaultIfEmpty()
                    select new
                    {
    ...
    //If nothing was found the response will be nocontent.
    if (properties == null) return Results.NoContent();
    ...
    //This will update the Command_Read
    Commands.Command_Read = a.Command_Read;
...
{
    //Search for all the commands that have command_performed null and also Command_read
    //not null also the command_string have to be diferent to RetrainModel.
    var properties = (
                            from Commands1 in db.Commands.DefaultIfEmpty()
                            select new
                            {
    ...
    //Update the Command_performed timestamp.
...
{
    //Search for all the Command that have the command_performed null
    //also have the command_read not null and Command_string is the same as RetrainModel.
    var properties = (
                        from Commands1 in db.Commands.DefaultIfEmpty()
                        select new
                        {
...
//Verify that if the last retrain model command was performed. 
...
{
    //str is the start time that the user will input 
    var str = DateTimeOffset.Parse(start).AddHours(4);
    //end is the end time that the user will input
